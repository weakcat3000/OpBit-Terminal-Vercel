...
const LYRA_POLL_INTERVAL_MS = 3000;
...
export class LyraPollStreamer {
    ...
    private restart(): void {
        this.stop();
        if (!this.underlying || !this.expiry || this.contractKeys.length === 0) {
            this.callbacks.onConnectionChange(false, 0);
            return;
        }

        this.fetchOnce();
        this.timer = setInterval(() => this.fetchOnce(), LYRA_POLL_INTERVAL_MS);
    }
    ...
    private async fetchOnce(): Promise<void> {
        ...
        try {
            ...
            for (const [targetExpiry, keys] of keysByExpiry) {
                ...
                if (!response.ok) {
                    this.callbacks.onConnectionChange(false, this.contractKeys.length, `LYRA poll HTTP ${response.status}`);
                    continue;
                }
                ...
                for (const row of rows) {
                    ...
                    const venue = row.venues?.LYRA_V2;
                    if (!venue) continue;

                    updates.push({
                        key: makeQuoteKey("LYRA_V2", row.contractKey),
                        seq: ++this.seq,
                        patch: {
                            bid: venue.bid ?? null,
                            ask: venue.ask ?? null,
            ...
            const now = Date.now();
            this.callbacks.onConnectionChange(true, this.contractKeys.length);
            ...
            this.callbacks.onConnectionChange(
                ...
                error instanceof Error ? error.message : "LYRA poll error"
            );
        }
    }
}
