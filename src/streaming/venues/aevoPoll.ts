...
const AEVO_POLL_INTERVAL_MS = 3000;
...
export class AevoPollStreamer {
    ...
    private restart(): void {
        ...
        this.fetchOnce();
        this.timer = setInterval(() => this.fetchOnce(), AEVO_POLL_INTERVAL_MS);
    }
    ...
    private async fetchOnce(): Promise<void> {
        ...
        try {
            ...
            for (const [targetExpiry, keys] of keysByExpiry) {
                ...
                if (!response.ok) {
                    this.callbacks.onConnectionChange(false, this.contractKeys.length, `AEVO poll HTTP ${response.status}`);
                    continue;
                }
                ...
                for (const row of rows) {
                    ...
                    const venue = row.venues?.AEVO;
                    if (!venue) continue;

                    updates.push({
                        key: makeQuoteKey("AEVO", row.contractKey),
                        seq: ++this.seq,
                        patch: {
                            bid: venue.bid ?? null,
                            ask: venue.ask ?? null,
            ...
            this.callbacks.onConnectionChange(
                ...
                error instanceof Error ? error.message : "AEVO poll error"
            );
        }
    }
}
